...
public partial class RegistroSuenoWindows : ContentPage
{
    private ExerciseDatabase baseDeDatos = new();
    private int idUsuarioActual;
    private bool usuarioAutenticado = false;
    private const double objetivoDiario = 8.0; // 8 horas recomendadas
    public ObservableCollection<RegistroSueno> RegistrosSueno { get; set; } = new();

    public RegistroSuenoWindows()
    {
        InitializeComponent();
    ...
    private async void RegistrarSueno_Clicked(object sender, EventArgs e)
    {
        if (!usuarioAutenticado || idUsuarioActual == -1)
        {
            await DisplayAlert("Error", "Debes iniciar sesión para registrar tu sueño.", "OK");
            return;
        }
        ...
        try
        {
            ...
            await DisplayAlert("Éxito", $"Has dormido {horasDormidas:F1} horas.", "OK");
    ...
    // Carga los ultimos 20 registros del historial
    private async Task CargarHistorialSueno()
    {
        try
        {
            ...
            historialTitleLabel.IsVisible = RegistrosSueno.Count > 0;

            // Sumar todas las horas registradas hoy
            ...
            // Cambiar mensaje según si se cumple el objetivo
    ...
    // Carga el gráfico semanal de sueño
